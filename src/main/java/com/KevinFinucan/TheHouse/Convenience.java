    ...
    */
    ...
    public int executeWinLossResults(int winnings, int currentBalance, int betAmount, boolean isWinner) {
        ...
        if (isWinner) {
            ...
            System.out.println("Oops, looks like The House won this bet. Better luck next time!");
            updatedBalance = currentBalance - betAmount;
        }
        return updatedBalance;
    }

    public int verifySelectionIsValidOption(int selection, int amountOfOptions, Scanner userInput) {
        while (selection < 1 || selection > amountOfOptions) {
            System.out.println("Oops, that selection does not match any of your options.");
            selection = promptForInt(userInput);
        }
        return selection;
    }
}
